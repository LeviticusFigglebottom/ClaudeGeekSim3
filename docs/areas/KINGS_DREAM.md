...
Everything accelerates. Once it has begun the Usher comes and stands across
the board, the closest he has ever been, and it is your move:

1. **Do nothing. Leave the board as it is.** Default.
2. **Concede. Lay the king down, and pull the plug. (an ending)** Asks
   again, "No" first. Off Air, `docs/areas/STATIC_END.md`.
3. **Checkmate. The pawn to the eighth, and stay on the board. (an
   ending)** Asks again. The Last Rank, `docs/areas/PROMOTION.md`.

Before the banquet has begun the board says it is not your move yet. The
cloth on the table is only the way back to the Anteroom
(`nexus:from_kings_dream`). See `docs/ENDINGS.md`.
