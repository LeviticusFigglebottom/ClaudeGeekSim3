...
AREAS=("$@")
if [ ${#AREAS[@]} -eq 0 ]; then
  AREAS=(apartment corridor hallway nexus forest city tavern house castle sea catacombs furnace cistern offices clocktower static mirror_nexus workshop kings_dream kings_mind pipes hospital promotion static_end)
fi
for a in "${AREAS[@]}"; do
